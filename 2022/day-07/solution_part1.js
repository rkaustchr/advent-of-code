...
function getTotalUnder(node, limit) {
    let total = 0;
    for(const key in node.children) {
        if ( node.children[ key ].type != "dir" )
            continue;

        if ( node.children[ key ].size <= limit )
            total += node.children[ key ].size;
        total += getTotalUnder(node.children[ key ], limit);
    }
    return total;
}
...
async function main() {
    ...
    const total = getTotalUnder(root, 100000);
