...
async function main() {
    ...
    // Scan the matrix left to right, storing if you have crossed a border of the loop
    //  everytime you cross a border you flip the in/out variable based on the openings and closings tiles
    //  drowing examples on papper,  |, L and J will count as crossing borders ( |, F and 7 also would work )
    let enclosedTiles = 0;
    for (let row=0; row < matrix.length; row++) {
        let inside = false;
        for(let col=0; col < matrix[0].length; col++) {
            if ( loop.has( getNodeKey(row, col) ) ) {
                // if you cross a border
                if ( matrix[row][col] == "|" || matrix[row][col] == "L" || matrix[row][col] == "J" )
                    inside = !inside;
                continue;
            }
    ...
    console.log("Result: %d", enclosedTiles);
