...
function validateOrder(seq, rules) {
    ...
    for (const curr of seq) {
        for (const prev of previous) {
            if (rules.get(curr).after.has(prev) || rules.get(prev).before.has(curr)) {
                seq.sort(getComparator(rules));
...
function getComparator(rules) {
    return (a, b) => {
        if ( rules.get(a).after.has(b) || rules.get(b).before.has(a) )
            return -1;
        if ( rules.get(a).before.has(b) || rules.get(b).after.has(a) )
            return 1;
        return 0;
    }
}
